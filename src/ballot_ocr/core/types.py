...
def _consume_magnitude(remaining: str, suffix: str, multiplier: int) -> tuple[int, str]:
    """Consume a magnitude (thousand, hundred) and return (value, remaining_text)."""
    for digit_thai, digit_val in [
        ("หนึ่ง", 1), ("สอง", 2), ("สาม", 3), ("สี่", 4), ("ห้า", 5),
        ("หก", 6), ("เจ็ด", 7), ("แปด", 8), ("เก้า", 9)
    ]:
        prefix = digit_thai + suffix
        if remaining.startswith(prefix):
            return digit_val * multiplier, remaining[len(prefix):]

    if remaining.startswith(suffix):
        return multiplier, remaining[len(suffix):]

    return 0, remaining


def _consume_tens(remaining: str) -> tuple[int, str]:
    """Consume tens place and return (value, remaining_text)."""
    if remaining.startswith("ยี่สิบ"):
        return 20, remaining[6:]
    ...
    for digit_thai, digit_val in [
        ...
        if remaining.startswith(prefix):
            return digit_val * 10, remaining[len(prefix):]
...
def _consume_units(remaining: str) -> int:
    """Consume unit place and return value."""
    if not remaining:
        return 0
    ...
    return THAI_DIGITS.get(remaining, 0)
...
def thai_text_to_number(thai_text: str) -> Optional[int]:
    ...
    # Handle magnitudes
    val, remaining = _consume_magnitude(remaining, "พัน", 1000)
    result += val
    val, remaining = _consume_magnitude(remaining, "ร้อย", 100)
    result += val

    # Handle tens
    val, remaining = _consume_tens(remaining)
    result += val

    # Handle units
    result += _consume_units(remaining)

    # Validate that we consumed anything and the parse made sense.
    # A successful parse must consume something and yield a result >= 0.
    # The 'ศูนย์' case is handled by exact matches.
    return result if (result > 0 or remaining != thai_text) else None


def validate_vote_entry(numeric: int, thai_text: str, confidence: float = 1.0) -> "VoteEntry":
...
class VoteEntry:
    ...
    confidence: float = 1.0  # Cell-level confidence
...
class BallotData:
    ...
    provenance_images: dict[str, str] = field(default_factory=dict)  # key -> path to cropped region

    def __repr__(self) -> str:
        station = self.polling_station_id or self.source_file
        return (
            f"BallotData({self.form_type!r}, "
    ...
    def from_dict(cls, d: dict) -> "BallotData":
        ...
        return cls(
            ...
            provenance_images=d.get("provenance_images", {}),
        )
