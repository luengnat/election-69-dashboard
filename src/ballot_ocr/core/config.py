...
Usage:
    from ballot_ocr.core.config import config
...
class Config:
    ...
    allowed_extensions: list = field(default_factory=lambda: ['.png', '.jpg', '.jpeg', '.pdf', '.zip'])
    ...
    def __post_init__(self):
        ...
        self.crops_dir = os.environ.get("CROPS_DIR", "crops")

    @property
    def auth_credentials(self) -> Optional[tuple[str, str]]:
        """Return auth tuple for Gradio if configured."""
        if self.web_ui_username and self.web_ui_password:
            return (self.web_ui_username, self.web_ui_password)
        return None
